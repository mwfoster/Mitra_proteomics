...
unenriched <- read.csv("wetblood_unenriched_MOFA.csv", header = TRUE, check.names = FALSE)
glyco <- read.csv("wetblood_glyco_MOFA.csv", header = TRUE, check.names = FALSE)
phos <- read.csv("wetblood_phospho_TiIMAC_MOFA.csv", header = TRUE, check.names = FALSE)
...
sample_groups <- c(
  rep("Subj09", 6),    # Samples 1-6
  rep("Subj10", 6),   # Samples 7-12
  rep("Subj11", 6),   # Samples 13-18
  rep("Subj12", 6),   # Samples 19-24
  rep("SPQC", 3)      # Samples 25-27
)
...
# 1. Create the plot with fixed axis and no extra space
plot_with_legend <- plot_factors(mofa, factors = 1:2, color_by = "group", dot_size = 3.5, alpha = 0.6) +
  coord_fixed(ratio = 1) +
  theme(
    axis.text = element_text(size = 13, face = "bold"),
    axis.title = element_text(size = 13, face = "bold"),
    plot.title = element_text(size = 13, face = "bold", hjust = 0.5),
    plot.margin = margin(0, 0, 0, 0),  # No space around plot
...
# 2. Extract and remove legend
legend <- get_legend(plot_with_legend)
plot_clean <- plot_with_legend + theme(legend.position = "none")

# 3. Combine with absolutely no space between plot and legend
final_plot <- plot_grid(
  plot_clean, legend,
  ncol = 2,
  rel_widths = c(1, 0.22),  # Adjust to taste
  align = "hv",
  greedy = TRUE
)
ggsave("latent_factors_plot_1_2.png", plot = final_plot, width = 4, height = 3, dpi = 600)

plot_data <- plot_factors(mofa, factors = 1:2, color_by = "group", return_data = TRUE)
write.csv(plot_data, "latent_factors_data.csv", row.names = FALSE)
...
plot <- plot_factors(mofa, factors = 1:2, color_by = "group", dot_size = 3.5, alpha = 0.6) +
  theme(
    ...
    legend.text = element_text(size = 12, face = "bold"),   # Legend label size and bold
...
plot_variance_factor <- plot_variance_explained(mofa, plot_total = FALSE) +
  theme(
    ...
    legend.text = element_text(size = 10, face = "bold"),   # Legend label size and bold
    legend.title = element_blank()   # Legend title size and bold
  )
...
# Extract the top 20 weights
top_weights <- head(sorted_weights, n = 50)
...
# Write to CSV
write.csv(top_weights_df, file = "top_50_weights_phospho_PTM_factor1.csv", row.names = FALSE)


# Load the dataset
phospho_data <- read.csv("top_50_weights_phospho_PTM_factor1.csv")  # Update with your actual file path
...
# Save the plot as a PNG file
ggsave("top_50_weights_phospho_PTMr_factor1.png", plot = phospho_plot, width = 16, height = 14, dpi = 600)
