...
nonlocalized_data <- "20250305_125405_6010_wetblood_TiIMAC_with_SPQC_prep_25_3_SN19_PTMreport_noloc.tsv"
nonlocalized_data <- read_tsv(nonlocalized_data)

# Importing the second TSV file
localized_data <- "20250305_125405_6010_wetblood_TiIMAC_with_SPQC_prep_25_3_SN19_PTMreport_loc.tsv"
...
```{r}
# Step 1: Identify columns with "Pool" in the name and "PTM.Quantity" in the header
spqc_columns <- grep("Pool", colnames(filtered_data), value = TRUE)
total_quantity_columns <- grep("PTM.Quantity", colnames(filtered_data), value = TRUE)

# Combine these two sets of columns (i.e., columns that contain both "Pool" and "PTM.Quantity")
...
write.table(filtered_data_nonredundant, "SICK_PTMsite_linear_10pct.tsv", sep = "\t", row.names = FALSE, quote = FALSE)

# Remove the last underscore and everything after it in PTM.CollapseKey
filtered_data_nonredundant$PTM.CollapseKey <- sub("_([^_]+)$", "", filtered_data_nonredundant$PTM.CollapseKey)

# Remove "_HUMAN" and "_BOVIN" from PTM.CollapseKey
filtered_data_nonredundant$PTM.CollapseKey <- gsub("_(HUMAN|BOVIN)", "", filtered_data_nonredundant$PTM.CollapseKey, ignore.case = TRUE)
...
write.table(filtered_data_subset, "Wetblood_TiIMAC_PTMsite_linear_10pct_MOFA.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
```
