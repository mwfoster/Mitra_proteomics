...
#localized_data$PTM.CollapseKey <- paste0(localized_data$PG.ProteinNames, "_", #(localized_data$PTM.CollapseKey, "(?<=_).*"))
...
filtered_data_nonredundant <- filtered_data %>%
  ...
  dplyr::inner_join(collapsed, by = group_cols)
...
write.table(filtered_data_nonredundant, "Accession_PI_LC_OT_TiO_PTMsite_all_columns.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
...
write.table(filtered_data_subset, "Accession_PI_LC_OT_TiO_PTMsite_impute.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
```
